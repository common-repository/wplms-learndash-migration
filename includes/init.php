...
class WPLMS_LEARNDASH_INIT{
    ...
    function build_curriculum($course_id){
    	...
    	$lessons_topics_quizzes = $wpdb->get_results("SELECT m.post_id as id,p.post_type as type,p.post_title as title FROM {$wpdb->postmeta} as m LEFT JOIN {$wpdb->posts} as p ON p.id = m.post_id WHERE m.meta_value = $course_id AND m.meta_key = 'course_id' ORDER BY p.$orderby $order");
                ...
                if(!empty($questions)){
                    foreach($questions as $question){
                        ...
                        if($question->answer_type != 'largetext' && $question->answer_type != 'assessment' && $question->answer_type != 'fillblank'){
                            preg_match_all("/_answer.;..[0-9]+.\"([A-Za-z0-9_ ]+)\"/", $question->answer_data, $output_array);
                            if(!empty($output_array)){
                                if($question->answer_type == 'sort'){
                                    $count = count($output_array[1]);
                                    $arr_reverse = array_reverse($output_array[1]);
                                    $arr = array();
                                    for($i=1; $i<=$count; $i++){
                                        $arr[] = $i;
                                    }
                                    $correct_answer = implode(',', $arr);
                                    update_post_meta($question_id,'vibe_question_options',$arr_reverse);
                                    update_post_meta($question_id,'vibe_question_answer',$correct_answer);
                                }else{
                                    update_post_meta($question_id,'vibe_question_options',$output_array[1]);
                                }
                            }

                            if($question->answer_type == 'match'){
                                preg_match_all('/_sortString\"...[0-9]+..([a-zA-Z0-9_ ]+)"/',$question->answer_data, $output_array);
                                if(!empty($output_array)){
                                    $content = get_post_field('post_content',$question_id);
                                    $match_list = '<br />[match]<ul>';
                                    $fields = array_reverse($output_array[1]);
                                    foreach($fields as $field){
                                        $match_list .='<li>'.$field.'</li>';
                                    }
                                    $match_list .= '</ul>[/match]';
                                    $content .= $match_list;
                            ...
                            if($question->answer_type == 'single' || $question->answer_type == 'multiple'){
                                preg_match_all("/_correct.;..([0-9]+)/", $question->answer_data, $output_array);
                                if(!empty($output_array)){
                                    update_post_meta($question_id,'vibe_question_answer',$output_array[1]);
                                }
                            }
                        }

                        if($question->answer_type == 'fillblank'){
                            preg_match_all("/_answer.;..[0-9]+.\".*\{(.*)\}.*\"/", $question->answer_data, $output_array);
                            if(!empty($output_array)){
                                $fillbank_replace = '{'.$output_array[1][0].'}';
                                $pos = strpos($output_array[1][0], '[');
                                if($pos === false){
                                    update_post_meta($question_id,'vibe_question_answer',$output_array[1][0]);
                                }else{
                                    preg_match_all("/\[(.*?)\]/", $output_array[1][0], $output);
                                    if(!empty($output)){
                                        $correct_answer = implode(',', $output[1]);
                                        update_post_meta($question_id,'vibe_question_answer',$correct_answer);
                                    }
                                }
                            }

                            preg_match_all("/_answer.;..[0-9]+.\"([^;]*)\";/", $question->answer_data, $outputs);
                            if(!empty($outputs)){
                                $content = get_post_field('post_content',$question_id);
                                $fill_blank = '<br />';
                                $fill_blank .= str_replace( $fillbank_replace, '[fillblank]',$outputs[1][0]);
                                $content .= $fill_blank;
                                $post = array('ID' => $question_id,'post_content' => $content );
                                wp_update_post($post,true);
                            }
